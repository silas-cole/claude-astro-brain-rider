...
class WakeWordDetector:
    ...
    def detect(self, audio_chunk_bytes) -> bool:
        ...
        if WakeWordDetector._log_counter % 100 == 0:
            logger.info(f"Prediction keys: {list(prediction.keys())}, values: {list(prediction.values())}")

        # Check predictions directly
        for key, score in prediction.items():
            # Log all scores periodically for debugging
            if score > 0.001:
                logger.debug(f"Wake word score: {key} = {score:.4f}")
            # Standard threshold - wake word typically scores 0.8+ when detected
            if score > 0.5:
                logger.info(f"Wake word DETECTED: {key} (Score: {score:.4f})")
                return True
